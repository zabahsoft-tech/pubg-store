                    ...
                    {posts.map(post => (
                        <Link key={post.id} to={`/blog/${post.slug}`} className="group bg-dark-800 rounded-xl overflow-hidden border border-dark-700 hover:border-brand-500/50 transition-all hover:shadow-xl hover:shadow-brand-900/10">
                            <div className="aspect-video overflow-hidden">
                                <img src={getStorageUrl(post.thumbnail)} alt={language === 'fa' ? post.fa_title : post.en_title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            </div>
                            <div className="p-6 space-y-4">
                                ...
                                <p className="text-gray-400 text-sm line-clamp-3 leading-relaxed">
                                    {stripHtml(language === 'fa' ? post.fa_description : post.en_description)}
                                </p>
